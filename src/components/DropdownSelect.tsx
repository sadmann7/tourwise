            ...
            >
              <Listbox.Options className="absolute z-10 mt-2 max-h-60 w-full overflow-auto rounded-md bg-neutral-900 py-1 text-sm shadow-lg ring-1 ring-gray-400 focus:outline-none">
                {options.map((option) => (
                  <Listbox.Option
                    key={option}
                    className="relative cursor-pointer select-none py-2 pl-10 pr-4 text-white transition hover:bg-neutral-700 ui-selected:bg-neutral-700 hover:ui-selected:bg-neutral-700"
                    value={option}
                  >
                    {({ selected }) => (
                      <>
